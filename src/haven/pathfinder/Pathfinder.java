...
public abstract class Pathfinder {
    ...
    private static GobHitbox.BBox plhb;

    static {
        plhb = GobHitbox.getBBox(UI.instance.gui.map.player());
    ...
    private boolean hitGob(final Coord mc) {
        ...
        for (xy.x = c.x; xy.x < br.x; xy.x += 1) {
            for (xy.y = c.y; xy.y < br.y; xy.y += 1) {
                if (ui.sess.glob.gobhitmap.checkHit(xy) && (ui.sess.glob.gobhitmap.getTileType(xy) != Tile.PLAYER)){
                    return (true);
                }
            }
        }
    ...
    private boolean hitOnBoat(final Coord mc) {
        ...
        for (xy.x = c.x; xy.x < br.x; xy.x += 1)
            for (xy.y = c.y; xy.y < br.y; xy.y += 1) {
                final Tile t = ui.sess.glob.map.gethitmap(xy.div(MCache.tilesz));
    ...
    final ArrayList<Move> advreduce(final List<Coord> lines) {
        if (lines != null) {
            ...
            for (int i = 0; i < lines.size() - 1; ++i) {
                ...
                blines.add(new Move(new Coord(lines.get(best))));
