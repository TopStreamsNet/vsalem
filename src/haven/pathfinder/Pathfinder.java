...
public class Pathfinder implements Runnable {
    ...
    public void pathfind(Coord src) {
        ...
        if (m.isOriginBlocked()) {
            ...
            mc = new Coord(src.x + freeloc.a - Map.origin, src.y + freeloc.b - Map.origin).floor();
        ...
        while (it.hasNext() && !moveinterupted && !terminate) {
            ...
            mc = new Coord(src.x + e.dest.x - Map.origin, src.y + e.dest.y - Map.origin).floor();

            if (action != null && !it.hasNext())
                mv.ui.gui.act(action);

            if (gob != null && !it.hasNext())
                mv.wdgmsg("click", gob.sc, mc, clickb, modflags, 0, (int) gob.id, gob.rc.floor(), 0, meshid);
            else
                mv.wdgmsg("click", Coord.z, mc, 1, 0);
